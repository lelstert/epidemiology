...
function learn_numerical_epidemiology_SIR(problem)
...
% typical time of recovery is 1/gamma, see initialization
    ...
    figure('Name','S Solution Daily','NumberTitle','off','position',scrsz)
        ...
        title(['The SIR Model Without Vital Dynamics :',' R0 = ',num2str(beta/gamma)])
    ...
    function dxdt=ode_SIR(~,x) % ~ means ignore argument for this function
        ...
        dxdt      =  zeros(3,1);
        dxdt(1,1) = -beta*x(2)*x(1)/N;               % rate of change in susceptible
        dxdt(2,1) =  beta*x(2)*x(1)/N - gamma*x(2);  % rate of change in infected
        dxdt(3,1) =  gamma*x(2);                     % rate of change of recovered 
